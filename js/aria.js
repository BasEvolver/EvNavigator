...
function initializeAriaPage() {
    ...
    // Default clean slate view for other personas
...
function renderAriaCleanSlate(persona) {
    ...
    switch(persona) {
        case 'adrian':
            suggestedPrompts = ["Compare NRR for CloudVantage vs. TechFlow.", "What are the key risks across the entire portfolio?", "Generate a summary of all Q2 board meetings."];
            break;
        case 'evelyn':
            suggestedPrompts = ["Prepare QBR materials for the NewCo acquisition.", "What is the budget impact of the AI feature delay?", "Draft a company-wide update on our Q2 performance."];
            break;
        case 'connor':
            suggestedPrompts = ["Analyze the key drivers of our Net Revenue Retention.", "Draft a revised sales comp plan.", "Which customers are the best candidates for the next upsell campaign?"];
            break;
    }

    ariaView.innerHTML = `
        <div id="aria-conversation-container" class="space-y-6"></div>
        <div id="aria-prompt-container" class="mt-6">
            ${getAdvancedPromptBoxHTML(suggestedPrompts)}
        </div>
    `;
}
...
function getAdvancedPromptBoxHTML(questions = []) {
    ...
    setTimeout(() => {
        ...
        const responseHTML = `
            <div id="${responseId}" class="aria-response-bubble">
                ...
                <div class="pl-11">
                    <div id="main-response-content-${responseId}">
                        ${responseData.renderFunc(state)}
                    </div>
                    <div id="actions-container-${responseId}" class="mt-6" style="opacity: 0; transform: translateY(20px);">
        ...
        setTimeout(async () => {
            ...
            if (responseElement) {
                // 1. Animate the main content first
                const mainContentContainer = document.getElementById(`main-response-content-${responseId}`);
                if (mainContentContainer) {
                    await runAriaBuildingSequence(mainContentContainer);
                }

                // 2. Animate the recommended actions only after the main content is finished
                const actionsContainer = document.getElementById(`actions-container-${responseId}`);
                if (actionsContainer && responseData.suggestedActions && responseData.suggestedActions.length > 0) {
                    await new Promise(r => setTimeout(r, 300)); // A brief pause for effect
                    actionsContainer.style.transition = 'opacity 0.4s ease, transform 0.4s ease';
                    actionsContainer.style.opacity = '1';
                    actionsContainer.style.transform = 'translateY(0)';
                } else {
                    // If there are no actions, remove the container so it doesn't take up space
                    actionsContainer?.remove();
                }
            }
            // 3. Finally, add the prompt box for follow-up questions
...
function handleFileAttachment(e) {
    ...
    displayContainer.innerHTML = ''; // Clear previous attachments
