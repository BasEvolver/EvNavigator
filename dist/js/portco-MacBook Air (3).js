...
function renderCroDashboard() {
    const mainContent = document.getElementById('main-content');
    const kpis = [
        { label: 'Global Quota Attainment', value: '103%', color: 'var(--status-success)' },
        { label: 'EMEA Quota Attainment', value: '75%', color: 'var(--status-error)' },
        { label: 'Pipeline Coverage', value: '3.5x', color: 'var(--text-primary)' },
        { label: 'Avg. Deal Size', value: '$85k', color: 'var(--text-primary)' }
    ];
    const kpisHTML = kpis.map(kpi => `<div class="kpi-card"><p class="kpi-label">${kpi.label}</p><p class="kpi-value" style="color: ${kpi.color};">${kpi.value}</p></div>`).join('');

    const topDeals = [
        { name: 'Apex Solutions', value: '$2.8M', stage: 'Negotiation', risk: 'Stalled (14 months in cycle)', riskClass: 'error' },
        { name: 'Stellar Technologies', value: '$925k', stage: 'Proposal', risk: 'Competing vs. AgileCloud', riskClass: 'warning' },
        { name: 'Global Enterprises Inc.', value: '$3.2M', stage: 'Verbal Commit', risk: 'Awaiting legal review', riskClass: 'success' }
    ];

    const actionItems = [
        { text: 'Approve 25% discount for Stellar Technologies', type: 'Discount Approval' },
        { text: 'Review revised MSA for Apex Solutions', type: 'Contract Review' }
    ];

    const strategicInitiatives = ceoDashboardData.departmentPlays.filter(p => ['Sales', 'Marketing', 'Partners'].includes(p.department));

    mainContent.innerHTML = `
        <h2 class="text-2xl font-bold mb-4">CRO Command Center</h2>
        <div class="persona-dashboard-layout">
            <div class="persona-main-column">
                <div class="portco-card">
                    <h3 class="card-title">Key Performance Indicators - Q2 2025</h3>
                    <div class="kpi-grid">${kpisHTML}</div>
                </div>
                <div class="portco-card">
                    <h3 class="card-title">Top Priority Deals</h3>
                    <div class="flex flex-col gap-4">
                        ${topDeals.map(deal => `
                            <div class="card-base p-4">
                                <div class="flex justify-between items-start">
                                    <div>
                                        <p class="font-bold text-primary">${deal.name}</p>
                                        <p class="text-sm text-secondary">${deal.stage}</p>
                                    </div>
                                    <p class="text-xl font-bold">${deal.value}</p>
                                </div>
                                <p class="text-xs mt-2 text-${deal.riskClass}"><strong>ARIA's Insight:</strong> ${deal.risk}</p>
                            </div>
                        `).join('')}
                    </div>
                </div>
                <div class="portco-card">
                    <h3 class="card-title">My Strategic Initiatives</h3>
                    <div class="program-list">${strategicInitiatives.map(play => {
                        const statusMap = { 'On Track': 'status-completed', 'At Risk': 'status-warning', 'Behind': 'status-error' };
                        return `<div class="program-item"><div class="program-name">${play.name}</div><div class="program-dept">${play.department}</div><div class="program-status-wrapper"><span class="program-status ${statusMap[play.status]}">${play.status}</span></div><div class="program-progress-container"><div class="program-progress-bar" style="width: ${play.progress}%"></div></div><div class="program-progress-text">${play.progress}%</div></div>`;
                    }).join('')}</div>
                </div>
            </div>
            <div class="persona-sidebar-column">
                <div class="portco-card">
                    <h3 class="card-title">My Action Items</h3>
                    <div class="flex flex-col gap-3">
                        ${actionItems.map(item => `
                            <div class="suggested-action-card !bg-bg-card">
                                <p class="font-semibold text-sm text-primary">${item.text}</p>
                                <p class="text-xs text-secondary">${item.type}</p>
                            </div>
                        `).join('')}
                    </div>
                </div>
                <div class="portco-card">
                    <h3 class="card-title">Playbook Guidance</h3>
                    <div class="analysis-box">
                        <p class="font-bold text-sm">Focus Area: NewCo Renewals</p>
                        <p class="text-secondary text-sm mt-1">Playbook recommends a <strong>20% price uplift</strong> for Gold-tier customers and <strong>10% for Silver</strong>. Prioritize securing these renewals before pursuing new cross-sell motions.</p>
                    </div>
                </div>
                <div class="portco-card">
                    <h3 class="card-title">AI Strategy Co-Pilot</h3>
                    <p class="text-secondary text-sm">For a deeper conversational analysis of your sales organization, please navigate to the ARIA assistant.</p>
                    <button class="primary-button mt-4 w-full" data-action="navigate" data-link="aria.html?company=cloudvantage">Go to ARIA</button>
                </div>
            </div>
        </div>
    `;
